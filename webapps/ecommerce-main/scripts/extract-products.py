...
ios_root = repository_root / 'xcode/eCommerce-main/eCommerce'

base = ios_root / 'Models/Product Info'
src = ios_root / 'Utilities/ProductData/Database/ProductDatabase.swift'
text = src.read_text()
...
out = webapp_root / 'src/data/products.ts'
...
# Extract discounts
discount_src = ios_root / 'Utilities/ProductData/Database/DiscountDatabase.swift'
...
discount_out = webapp_root / 'src/data/discounts.ts'
...
# Extract new ins
newin_src = ios_root / 'Utilities/ProductData/Database/NewInDatabase.swift'
newin_text = newin_src.read_text()
newins = []
for m in re.finditer(r'NewIn\(\s*id:\s*"([^"]+)"\s*\)', newin_text):
    newins.append(m.group(1))

newin_out = webapp_root / 'src/data/newins.ts'
