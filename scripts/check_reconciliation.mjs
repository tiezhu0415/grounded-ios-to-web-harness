...
const stateIds = new Set(screens.flatMap((screen) => (screen.states || []).map((state) => state.id)));
const visualStateIds = new Set((matrix.screens || []).flatMap((screen) => (screen.states || []).map((state) => state.id)));
...
for (const screen of screens) {
  ...
  for (const stateId of flows) if (!visualStateIds.has(stateId)) errors.push(`${label} references unknown runtime visual state: ${stateId}`);
...
for (const candidate of candidates.values()) {
  ...
  if (candidate.status === 'MAPPED') {
    ...
    if (!Array.isArray(candidate.state_ids) || candidate.state_ids.length === 0) errors.push(`mapped state candidate has no Screen+State: ${label}`);
    for (const id of candidate.screen_ids || []) if (!screenIds.has(id)) errors.push(`state candidate references unknown screen: ${id}`);
    for (const id of candidate.state_ids || []) {
      if (!stateIds.has(id)) errors.push(`state candidate references unknown Screen+State: ${id}`);
      const state = screens.flatMap((screen) => screen.states || []).find((item) => item.id === id);
      if (state && !(state.source_evidence || []).includes(candidate.id)) {
        errors.push(`mapped Screen+State does not cite its source candidate: ${candidate.id} -> ${id}`);
      }
    }
  }
}
  ...
  for (const state of (screen.states || []).filter((item) => item.required === true)) {
    ...
    for (const evidence of state.source_evidence || []) {
      if (evidence.startsWith('ALWAYS_VISIBLE:')) {
        const ref = evidence.slice('ALWAYS_VISIBLE:'.length);
        const coverageScreen = screens.find((item) => item.id === screen.source_id);
        const allowed = new Set([screen.source_id, coverageScreen?.id, `${coverageScreen?.source}#${coverageScreen?.declaration}`]);
        if (!allowed.has(ref)) errors.push(`${label} has mismatched ALWAYS_VISIBLE evidence`);
