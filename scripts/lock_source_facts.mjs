...
import { stateSnapshotsPayload } from './state_snapshots.mjs';
...
const errors = validateSourceFacts({ coverage, facts, matrix, snapshots, requireLocked: false, allowUnresolved: false });
...
const previous = facts.lock?.status === 'LOCKED' ? {
  ...
  state_snapshots_sha256: facts.lock.state_snapshots_sha256,
...
facts.lock.state_snapshots_sha256 = sha256Json(stateSnapshotsPayload(snapshots));
